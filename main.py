...
fps = 500  # Setting the maximum frames per second
...
def start_menu():
    ...
    new_game = NewGameButton(half_width, half_height - 50, image_cache)
    quit_game = QuitGameButton(half_width, half_height + 10, image_cache)
...
def draw_level(x, y, column, obsticles, entities, image_cache):
    # Grass
    if column == "G":
        image = functions.get_image("data/images/Grass1.png", image_cache)
        tile = EnvironmentTile(x, y, 32, 32, "Grass", "Grass", image)
        grass.append(tile)
        entities.add(tile)
    # Plain Grass
    elif column == "P":
        image = functions.get_image("data/images/Grass_Plain.png", image_cache)
        tile = EnvironmentTile(x, y, 32, 32, "Grass", "Plain Grass", image)
        grass.append(tile)
        entities.add(tile)
    # Grass with yellow flower
    elif column == "F":
        image = functions.get_image("data/images/Grass_Flower.png", image_cache)
        tile = EnvironmentTile(x, y, 32, 32, "Grass", "Flower Grass", image)
        grass.append(tile)
        entities.add(tile)
    # Grass To Sand (50/50 split block) Direct
    elif column == "Y":
        image = functions.get_image("data/images/Grass_To_SandD.png", image_cache)
        tile = EnvironmentTile(x, y, 32, 32, "Grass", "Grass To Sand", image)
        grass.append(tile)
        entities.add(tile)
    # Tree
    elif column == "T":
        image = functions.get_image("data/images/Tree.png", image_cache)
        tile = EnvironmentTile(x, y, 32, 32, "Tree", "Tree", image)
        trees.append(tile)
        entities.add(tile)
    # Path
    elif column == "O":
        image = functions.get_image("data/images/Path.png", image_cache)
        tile = EnvironmentTile(x, y, 32, 32, "Path", "Path", image)
        grass.append(tile)
        entities.add(tile)
    # Sand
    elif column == "S":
        image = functions.get_image("data/images/Sand.png", image_cache)
        tile = EnvironmentTile(x, y, 32, 32, "Sand", "Sand", image)
        grass.append(tile)
        entities.add(tile)
    # Sand To Water (50/50 split block) Direct
    elif column == "Q":
        image = functions.get_image("data/images/Sand_To_Water.png", image_cache)
        tile = EnvironmentTile(x, y, 32, 32, "Sand", "Sand To Water", image)
        entities.add(tile)
    # Water#
    elif column == "W":
        image = functions.get_image("data/images/Water.png", image_cache)
        tile = EnvironmentTile(x, y, 32, 32, "Water", "Water", image)
        entities.add(tile)
    elif column == "L":
        image = functions.get_image("data/images/Wood.png", image_cache)
        tile = EnvironmentTile(x, y, 32, 32, "Wood", "Wood", image)
        entities.add(tile)
...
def replace_block(mouse_pos):
    ...
    for g in grass:
        ...
        if rect.collidepoint(mouse_pos[0], mouse_pos[1]):
            ...
            new_image = EnvironmentTile(g.rect[0], g.rect[1], 32, 32, "Dug Out", "Dug Out",
                                        functions.get_image("data/images/Dug_Out.png", image_cache))
            grass.remove(g)
